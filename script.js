...
function initJeu() {
    ...
    container.innerHTML = ""; // Sécurité reset
...
function verifierAtelier(original) {
    ...
    // Nettoyage de la saisie (espaces et accents)
    const val = input.value.trim().toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    const target = original.toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    ...
    if (val === target) {
        ...
        input.blur(); // Ferme le clavier pour confirmer la réussite
    }
}
